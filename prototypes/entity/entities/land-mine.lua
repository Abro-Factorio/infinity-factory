...
local function object_updater(zero_object, level, object)
    object.trigger_radius = zero_object.trigger_radius * math.pow(Config.upgrade_multiplier, level)
    ...
    get_from_array_by_type(object_source_effects, "nested-result").action.radius =
        get_from_array_by_type(zero_object_source_effects, "nested-result").action.radius * math.pow(Config.upgrade_multiplier, level)
